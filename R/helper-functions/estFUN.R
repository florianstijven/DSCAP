estFUN_taudelta_CC <- function(data, models_tbl, target_trial, estimate_weights, weights_df){
  ...
  ncov <- length(names(models_tbl$outcome_model_fit_Y[[1]]$coefficients))
  covnames <- names(models_tbl$outcome_model_fit_Y[[1]]$coefficients)[2:ncov]
  ...
  tau_1_to_ntrial_pos = (trt_effect_start + (1 + n_trials) * 2):(trt_effect_start + (1 + n_trials) * 3 - 2)
  ...
  function(theta, estimate_weights){
    ...
    stacked_ee <- c(
      ...
      clin_stand_parm_ee,
      surr_stand_parm_ee,
      (1 - theta[clin_stand_parm_pos[-c(placebo_number + 1)]] / theta[clin_stand_parm_pos[placebo_number + 1]]) - theta[tau_1_to_ntrial_pos], #tau 1 to 8
      # causal association pearson rho
      cor(theta[tau_1_to_ntrial_pos], theta[surr_stand_parm_pos]) - theta[corr_pos[1]],
      # causal association parameter beta -- linear regression slope
      (cov(theta[tau_1_to_ntrial_pos], theta[surr_stand_parm_pos]) / 
...
estFUN_taudelta <- function(data, models_tbl, target_trial){
  n_trials = nrow(models_tbl)
  ...
  # Estimating function for outcome regression model for the surrogate outcome
  # for the patient's trial.
  surr_or_psiFUN_trial_A = if (trial_subject %in% c("Placebo", target_trial)) {
    # If the surrogate is missing, the estimating function is zero.
    function(theta)
      return(rep(0, ncov))
  } else {
  ...
  function(theta){
    ...
    # parameters.
    surr_stand_parm_ee = c(
      (1 - R) * (A != 0) * (S - theta[surr_stand_parm_pos[1]]),
      (1 - R) * (predicted_S_all_trials - theta[surr_stand_parm_pos[-1]])
    )
